...
from src.FaceMEO.openai_wrapper import first_prompt_sequence, query_model, initialize_preprompts
...
# def run_gpt(prompt_sequence, context):          
    ## TODO 
...
if __name__ == "__main__":
    ...
    ## initialization
    initial_prompt_sequence = initialize_preprompts(initial_prompt_sequence)
    # face_model = face_model_io.load_face_model('/input/inyup/ICT-FaceKit/FaceXModel')
    
    while True:
        user_input = input("You: ")
        if user_input.lower() in ["exit", "quit"]: # lower() : 소문자로
            break
        
        conversation_history.append({"role" : "user", "content" : f"# {user_input}\n"}) # this is a openAI for mat
        if motion_id == 0:
            prompt_sequence = first_prompt_sequence(initial_prompt_sequence, [f"# {user_input}\n"]) # list of str
        else:
            # import pdb;pdb.set_trace()
            # prompt_sequence = conversation_history # currently list of dict
            prompt_sequence = [entry['content'] for entry in conversation_history]
        
        # import pdb;pdb.set_trace()
        c, r, context = query_model(prompt_sequence, prompt_sequence, 0) # code = responses
        conversation_history.append({"role" : "assistant", "content" : c})        
        print(f"chatbot : {c} \n")
...
The person is talking. Make a face when a kid figured out Santa doesn't exist in the middle of the sequence.
The person is talking. You are watching a villain killing innocent people. Make the face throughout the entire sequence.
The person is talking. Make angry face entire sequence. Blink slowly when mouth is max opened. move that blink to the middle of the sequence.
"""