...
class FacialMotion():
    ...
    def insert_keyframe(self): # interpolate 잘 안되는중 디버깅 잘 살펴보기, 아무래도 precision 때문인 것 같음. 너무 잘게 나눠서 interpolate 값이 smooth하게 적용이 안됨
        ...
        """
        output_sequence = self.source_animation_seq.copy() # 완전 처음엔 0인 상황이다 -> keyframe을 넣어야한다 
        ...
        if start_idx - num_front <= 0: # check if enough front_speed frames are available, if not, decrease front_speed to allowalbe length
        ...
        if end_idx + num_back  >= self.seq_len - 1: # check if enough back_speed frames are available, if not, decrease back_speed to allowalbe length
            num_back = (self.seq_len - 1)- (end_idx) - 1  # so that the last frame should always be from source sequence
