...
emotions = {
    "angry": [
        ...
        'jaw_thrust',         # AU29: Push the jaw forward
...
class FacialMotion():
    def __init__(self, motion = None):
        ...
        # self.motion_length = 110 # later to be updated to automatically set
        self.motion_length = 110
        ...
        self.default_intensity_val = 1.0  # set to 0.5 as default
        if type(motion) == dict:
            self.source_animation_seq = motion["output_animation_seq"]
            self.key_exp_parameter = motion["key_exp_parameter"]
        elif motion == None:
            self.source_animation_seq = np.zeros((self.motion_length,53), dtype=np.float64) # set to zeros as default
        else:
            self.source_animation_seq = motion.output_animation_seq
            self.key_exp_parameter = motion.key_exp_parameter
        self.output_animation_seq = np.zeros((self.motion_length,53), dtype=np.float64)
        
        self.intensity_vector = np.ones(53) * self.default_intensity_val # (53,) vector set to 0.5 as default
        self.cur_activated = []
        self.replace_activated = True # default is adding a new keyframe, so set to True 
        self.seq_len = self.source_animation_seq.shape[0]
    
    def get_key_frame(self):
        return self.source_animation_seq[self.key_idx]
    ...
    def when_bshp(self, at : str = "max", point : str = None):
        ...
        if at == "max":    
            ...
            for i in range(self.seq_len): 
                frame_val = self.source_animation_seq[i][idx] # find corresponding bhsp targets' value
            ...
            min_val = 1000
            for i in range(self.seq_len): 
                frame_val = self.source_animation_seq[i][idx]
    ...
    def set_activated(self, activated_list):
        ...
        for activated in activated_list:
            ...
            if activated in self.cur_activated:
                ...
                final_list.append(activated)     
        if self.replace_activated:
            self.cur_activated = final_list
        else:
            self.cur_activated += final_list    
    ...
    def set_speed(self, speed_locations : list, speed_frames : list, replace=False): 
        ...
        for i, speed_location in enumerate(speed_locations):
            ...
            if replace:
                if speed_location == "keyframes":
                    if speed_frames[i] < self.seq_len: # if value is self.seq_len or over, leave it as it is
                        speed_frames[i] = int(speed_frames[i] / 2)
                self.speed[speed_location] = speed_frames[i]
            else:
                self.speed[speed_location] = int(self.speed[speed_location] * speed_frames[i])
        
    def set_key_exp(self, intensities : list = None, replace=True): 
        """
        after putting into cur_activated,
        get keyframe expression with given intensities
        """
        self.key_exp_parameter = self.get_key_frame()
        for i, c_a in enumerate(self.cur_activated):
            id = full_blendshape_targets.index(c_a)
            if self.key_exp_parameter[id] == 0:
                self.key_exp_parameter[id] = 1 # activate 
            if intensities == None:
                continue # leave it as default
            else:
                if replace:
                    self.key_exp_parameter[id] = intensities[i]
                else:
                    self.intensity_vector[id] *= intensities[i]
                    self.key_exp_parameter[id] = self.intensity_vector[id]  # Update key_exp_parameter

        print(f"keyframe : {self.key_exp_parameter}\n")
    ...
    def activate_hierarchical(self, identifiers : list, intensities: list = None, replace=True):
        """
            ...
            if identifier in emotions.keys():
                ...
                for facs in facs_units:
                    ...
                    for bt in blendshape_targets:
                        if bt not in expanded_identifiers:
                            expanded_identifiers.append(bt)
                ...
                if identifier not in expanded_identifiers:
                    expanded_identifiers.append(identifier)
                expanded_intensities.append(intensities[identifiers.index(identifier)])
            else:
                raise ValueError("There's not such identifier in the current pipeline")
        
        self.set_activated(expanded_identifiers)    
        self.set_key_exp(expanded_intensities, replace=replace)
    ...
    def insert_keyframe(self, key_idx_pos="center"):
        output_sequence = self.source_animation_seq.copy()
        
        duration = self.speed["keyframes"]
        num_front = self.speed["front"]
        num_back = self.speed["back"]

        if key_idx_pos == "center":
            start_idx = max(0, self.key_idx - duration)
            end_idx = min(self.seq_len - 1, self.key_idx + duration)
        elif key_idx_pos == "start":
            start_idx = self.key_idx
            end_idx = min(self.seq_len - 1, self.key_idx + 2 * duration)
        elif key_idx_pos == "end":
            start_idx = max(0, self.key_idx - 2 * duration)
            end_idx = self.key_idx
        else:
            raise ValueError(f"Invalid keyframe_position: {key_idx_pos}. Expected 'center', 'start', or 'end'.")
        
        start_idx = max(0, min(start_idx, self.seq_len - 1))
        end_idx = max(0, min(end_idx, self.seq_len - 1))
        
        for i in range(start_idx, end_idx + 1):
            output_sequence[i] = self.key_exp_parameter  
        
        if start_idx > 0:
            front_start_idx = max(0, start_idx - num_front)
            for i in range(1, start_idx - front_start_idx + 1):
                t = i / (start_idx - front_start_idx + 1)
                output_sequence[start_idx - i] = (1 - t) * output_sequence[start_idx] + t * output_sequence[front_start_idx]

        if end_idx < self.seq_len - 1:
            back_end_idx = min(self.seq_len - 1, end_idx + num_back)
            for i in range(1, back_end_idx - end_idx + 1):
                t = i / (back_end_idx - end_idx + 1)
                output_sequence[end_idx + i] = (1 - t) * output_sequence[end_idx] + t * output_sequence[back_end_idx]

        return output_sequence
    
    def insert_keyframe_old(self):
        """
        source animation sequence의 특정 point에 keyframe이(blendshape 파라미터로 이뤄진 특정 표정)이 삽입될 것  
        keyframe의 duration: keyframe이 self.key_idx 기준 앞뒤 몇 프레임씩 인지
        keyframe의 front: keyframe의 앞 frame들과 몇 프레임에 걸쳐 interpolate되는지 
        keyframe의 back: keyframe의 뒷 frame들과 몇 프레임에 걸쳐 interpolate되는지 
        """
        output_sequence = self.source_animation_seq.copy()
        keyframe_sequence = np.zeros_like(output_sequence) # N,53
        duration = self.speed["keyframes"] 
        num_front = self.speed["front"] 
        num_back = self.speed["back"] 

        for i in range((duration * 2) + 1):
            index = self.key_idx - duration + i
            if index < 0 or index >= self.seq_len:
                continue
            keyframe_sequence[index] = self.key_exp_parameter
            
        if self.key_idx + duration >= self.seq_len:
            end_idx = self.seq_len - 1
        else: 
            end_idx = self.key_idx + duration
        
        if self.key_idx - duration < 0: 
            start_idx = 0
        else:
        ...
        if start_idx == end_idx: 
            keyframes = keyframe_sequence[start_idx] # 53,
        else:   
            keyframes = keyframe_sequence[start_idx : end_idx + 1] # num_keyframes,53
        ...
        if start_idx - num_front <= 0:
            num_front = start_idx - 1
            front_speed_frame = output_sequence[0]
        else:
            front_speed_frame = output_sequence[start_idx - (num_front + 1)]
        ...
        if end_idx + num_back  >= self.seq_len - 1: 
            num_back = (self.seq_len - 1)- (end_idx) - 1  
            back_speed_frame = output_sequence[-1]
        else:
            back_speed_frame = output_sequence[end_idx + (num_back) + 1]
